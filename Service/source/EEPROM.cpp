...
void EEPROM::Store(const int8* data, int8 length, uint32 address)
{
...
void EEPROM::Load(int8 *data, int8 length, uint32 address)
{
  address += EEPROM_START;
  for(uint8 i=0; i<length; i++)
...
void EEPROM::Store(int8 value, uint32 address) 
{ 
  eeprom_write_byte((unsigned char *) address, value);
}

int8 EEPROM::Load(uint32 address)
{
